...
const ClassRoom = () => {
  ...
  const handleUpdateQuestionStatus = async (questionId, status) => {
    try {
      const response = await questionAPI.updateQuestionStatus(questionId, status);
      setQuestions(prev => 
        prev.map(q => 
          q._id === questionId 
            ? { ...q, status: status }
  ...
  const canModerateQuestions = user?.role === 'instructor' || user?.role === 'ta';
      ...
      <main className="classroom-main">
        <div className="classroom-header">
          ...
          <div className="class-title">
            ...
            <p>Ask and answer questions in real-time</p>
          </div>
